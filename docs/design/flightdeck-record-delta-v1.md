...
Contract-ready: 2026-09-05. Producer implementation and isolated validation are complete for manager review; do not enable before successful capability negotiation. Existing `/sync` and its cursors remain unchanged.
...
`{protocol_version:1, families:string[], mode:'snapshot'|'delta', changes:Change[], actors?:Actor[], next_cursor:string, has_more:boolean, snapshot_id:string|null, snapshot_complete:boolean, partitions_complete:string[], bounds:{max_rows:200,max_bytes:1048576}}`.
...
## Visible actor sidecar (integration supplement)

New v1 producers always emit `actors` (including `[]`); it is optional in types/OpenAPI for compatibility with older v1 producers and consumers. Exact actor shape: `{actor_id:string, npub:string, display_name:string|null, kind:'human'|'agent'|'app'|'service'}`. IDs are deduplicated per page. Only non-null typed top-level references in **returned visible upserts** are resolved: `created_by_actor_id`, `updated_by_actor_id`, `deleted_by_actor_id`, `owner_actor_id`, `actor_id` (task assignment), `viewer_actor_id`. Arbitrary metadata is never traversed. Hidden entries and tombstones add no identities. This provides identity rendering, not workspace membership, roles, grants, or group edges.

`changes.length + actors.length <= 200`; `limit` still bounds scanned canonical entries, and metadata lookahead remains one. Actors are included in the full 1 MiB JSON bound. The next row and its new identities are admitted together or deferred without advancing over that row. A single row plus required actors that exceeds the byte bound returns 413. Actor label byte sizes are checked in SQL before identity payload fetch. Fixture pages include usable actors without preseeded members.

A channel/task/doc-only member can bootstrap from record-sync and translate senders/assignees using this page sidecar; it needs no `/members` call. Merge actors and canonical changes in the same Dexie transaction. `/members` and group directory retain their independent authorization: restricted consumers can safely skip both for canonical transcript/task/doc rendering. Group management and full membership pickers still require separately authorized directory APIs. Do not infer membership or remove cached identities from sidecar omission. Actor labels are current at page read, not historical journal labels; profile-only edits do not emit record deltas, so idle cached labels may remain stale until the actor is referenced again or an authorized directory refresh occurs. Identity/ACL resets still discard the authoritative generation.

## Effective transcript query continuation

The existing `GET .../channels/:channelId/messages?thread_id=...&effective_transcript=true` now pushes its `limit+1` into indexed effective-lineage ranges instead of materializing and slicing full histories. Each fork anchor is resolved by ID, including deleted anchors and inherited ancestor points, with the existing 100-thread/cycle checks. The existing millisecond timestamp/ID cursor, lineage order, inherited/read-only fields and tombstone serialization are unchanged. Branch creation validates only its requested message ID through those ranges. This legacy endpoint has a row bound, not the record-sync 1 MiB byte guarantee. See the inventory and `flightdeck-branches-benchmark.json` for query evidence and limits.

Delta SQL orders by the underlying bigint journal position, explicitly qualified to avoid sorting the text wire-version alias lexicographically. The row-expansion regression crosses decimal position boundaries and verifies no lost or repeated page entries.
